...
public class Controller : MonoBehaviour
{
    ...
    private Dictionary<GameObject, Dog> allDogs = new Dictionary<GameObject, Dog>(); //!< A dictionary of all the player's dogs with their GameObjects as their keys for ease of reference.
    private List<Dog> orderedListOfDogs = new List<Dog>(); //!< An ordered list of all the player's dogs for iterating through.
    public int dogLimit = 0;                               //!< Maximum number of allowed dogs.
    ...
    [SerializeField] private Text dogFocusButtonName = null; //!< Reference to the name text on the dog focus button to display the current dog the button would cause the player to focus on.
    private int currentFocusButtonDog = 0;                   //!< Current dog the focus button would focus on when clicked.
    ...
    */
    public void AddDog(Dog newDog)
    {
        ...
        cameraController.SetFocusDog(newDog);
        SetFocusButtonDog(true);
    }
    ...
    */
    ...
    /** \brief Called when the next/back buttons for the dog focus button are pressed, to get the next dog in the ordered list. 
    */
    public void SetFocusButtonDog(bool next)
    {
        switch (next)
        {
            case (true):
                if (currentFocusButtonDog < allDogs.Count - 1)
                {
                    currentFocusButtonDog++;
                }
                else { currentFocusButtonDog = 0; }
                break;
            case (false):
                if (currentFocusButtonDog > 0)
                {
                    currentFocusButtonDog--;
                }
                else { currentFocusButtonDog = allDogs.Count - 1; }
                break;
        }

        dogFocusButtonName.text = orderedListOfDogs[currentFocusButtonDog].name;
    }

    public void FocusDogButtonClick()
    {
        cameraController.SetFocusDog(orderedListOfDogs[currentFocusButtonDog]);
    }
}
